...
Note: 1) Accessing the Microservice Entrypoint will load, by default, the user documentation and you can  follow the instructions to use the microservice. 2) The first time you start the application, if the Mongo Docker image is running properly, a new database `test` with t
