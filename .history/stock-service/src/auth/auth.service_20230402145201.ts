...
export class AuthService {
  ...
  async resetPassword(user: CreateUserDto): Promise<any|Message> {
    ...
    const reset = await this.usersService.validadeUser(user.email, user.password); // Validate user credentials
    if ( reset.statusCode === 400 ) { // User or Password is incorrect.
      return validate
    }
  }
