...
There commands will download, install and run a local MongoDB Docker Instance. We are giving an user and password to login to mongo for secutiry You can check if the instance was created properly by executing the command:
...
Use that information provided to configure the MongoDB environment constants on the application. Do it on the file `mongo.config.ts` on the `src` directory on the project root.
