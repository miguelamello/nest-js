...
export class StocksController {
  ...
  async getHistory(
    ...
    if (userId) {
      const response = await this.stocksService.getHistory(userId);
      if (response.length) {
        for (const item of collection) {
          // Perform your transformations on each item here
          const { name, symbol, open, close } = item;
          console.log(`Name: ${name}, Symbol: ${symbol}, Open: ${open}, Close: ${close}`);
        }
        return response
      }
      return []
