...
export class UsersService {
  ...
  async resetPassword(userid: string): Promise<any|Message> { 
    const newpassword = this.utilService.getHash();
    const user = await this.userModel.findByIdAndUpdate(userid.toString(), { password: newpassword }, { new: true });
    return (user) ? user : { statusCode: 400, message: 'Password not reseted.' };
  }
